...
// otherwise. Dropping every implicit argument for a sub-command matches the
// CLI: built-in kits carry permission flags there, and a sub-command such as
// "claude agents" rejects them.
type Recipe struct {
	...
	// sub-command. FromManifest leaves it empty: the manifest's flat fields
	// do not say which arguments are fixed.
...
// FromManifest resolves the recipe for a fresh session of m, which should be
// the effective (inheritance-resolved) manifest of the sandbox kit.
//
// A manifest with no Binary gives a KindShell recipe. Otherwise DefaultArgs
// is m.RunOptions, the options the docker/sandboxes CLI adds when it starts
// the agent in a Cloud session, so the script runs the same argv.
func FromManifest(m *spec.Manifest) Recipe {
	...
	return Recipe{
		...
		Kind:        KindAgent,
		Binary:      m.Binary,
		DefaultArgs: append([]string(nil), m.RunOptions...),
	}
}
...
func (r Recipe) Validate() error {
	...
	switch r.Kind {
	case KindShell:
		if r.Binary != "" || len(r.FixedArgs) > 0 || len(r.DefaultArgs) > 0 {
...
// script is already running in a bash that read it at startup, as when a
// client runs "bash <launcher>". The agent keeps the caller's user.
//
