#!/bin/sh
# Entrypoint. Applies the resolved Anthropic auth state, then execs Aider.
#
# Invoked through `sh` because files under `files/home/` land 0644: the
# artifact loader does not carry the source exec bit.
set -u

AUTH_ENV_FILE="$HOME/.config/aider/anthropic-auth.env"
# Written by aider-anthropic-auth.sh at container start; absent means the
