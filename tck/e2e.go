...
func RunE2EKit(t *testing.T, kitPath string, opts E2EOptions) {
	...
	t.Run(filepath.Base(absKit), func(t *testing.T) {
		...
		name := createSbx(t, ctx, absKit, agent, td, opts.AppName)
...
func createSbx(t *testing.T, ctx context.Context, absKit, agent string, td *kitTCKData, appName string) string {
	t.Helper()
	workspace := t.TempDir()
	...
	createOut, err := runSbx(t, ctx, appName, "create", "--kit", absKit, "--name", name, agent, workspace)
