...
```console
sbx run -p 8000 "git+https://github.com/docker/sbx-kits-contrib.git#dir=openhands-canvas" .
sbx run -p 8000 ./openhands-canvas/ .
```
...
The v3 descriptor is `openhands-canvas.yaml`; its companion content recipe is
`openhands-canvas.dockerfile`.
