Claude Code is installed at `~/.local/bin/claude`, which this sandbox's base
already exports on `PATH`. Run `claude` for an interactive session or
`claude -p "<prompt>"` for a one-shot answer — this kit contributes the agent,
not the launch command, so the base workload's own entrypoint is unchanged.

Authentication is proxy-mediated and covers both an Anthropic console API key
and a Claude subscription, whichever the host has bound under the `anthropic`
service (`sbx secret set anthropic`). The container never holds a usable
credential: a bound key is substituted by the proxy on outbound requests, and a
bound subscription arrives as sentinel tokens in `~/.claude/.credentials.json`
that the proxy swaps per request. With nothing bound, `claude` falls back to its
own `/login`.
