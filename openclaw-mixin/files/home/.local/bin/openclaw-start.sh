...
# the gateway up -- a lifecycle startup hook runs on every container start, not
# just create -- so this waits on the readiness sentinel and drops into the TUI.
#
...
# since a non-zero startup hook neither fails `sbx create` nor prints anything.
set -e
