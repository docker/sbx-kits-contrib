...
# A startup hook re-runs on every container start (the lifecycle@1 contract)
# and $STATE_DIR survives a stop/start, so last boot's sentinel outlives the
...
# (see the README) races into the very failure it exists to prevent.
...
#
# The sentinel must match the kit descriptor's credential
# oauth.sentinels.accessToken.
