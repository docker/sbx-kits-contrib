...
# The kit's `version` arg arriving as a build arg -- openhands.yaml declares it
# with `buildArg: OPENHANDS_VERSION`, validates its shape, and expands the same
# value into `provides: ["openhands@..."]`. Deliberately no default here: the
# descriptor is the single source of the pin, and a second default in this file
# would be one more thing to keep in sync with it.
...
#
# The requirement is `openhands==<version>`, an exact specifier rather than a
# range, so what uv resolves is the release the descriptor promised -- which is
# what makes the provide's version true of the built image.
#
